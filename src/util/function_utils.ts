import { Culture, CultureToPersist } from "../contexts/CultureContext";
...
export function getSpecificCulture(cultures: Culture[], cultureId: string): Culture | undefined {
    var culture: Culture | undefined
    cultures.forEach((currentCulture) => {
        if (currentCulture.id == cultureId) {
...
export async function calculate(culture: Culture) {
    const startDate = new Date(culture.startDate)
...
export function mapCulturesToCulturesToPersist(cultures: Culture[]) {
    const culturesToPersist: CultureToPersist[] = cultures.map((culture) => {
        return {
            id: culture.id,
            planta: culture.planta,
            description: culture.description,
            startDate: culture.startDate,
            localization: culture.localization,
        }
    })
    return culturesToPersist
}
